...
public class FrontController extends HttpServlet {
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //リクエスタからパラメータ”action”の値を取得（例："Employee","Report"）
            ...
            type = Class.forName(String.format("actions.%Acgion", actionString));

            //ActionBaseのオブジェクトにキャスト（例：action.EmployeeActionオブジェクト→actions.ActionBaseオブジェクト）
            ...
            //リクエストパラメータに設定されている"action"の値が不正の場合（例：action=xxxxなど、該当するActionクラスがない場合）
